...
**v1.0.10.0**
- Re-centered the action/category icons — the bounding box was already perfectly centered, but the pose's visual weight (bulkier on one side from the shoulder pads) sat about 5% of the icon's width off-center, which read as visibly off to the eye even though the math checked out. Now centered on visual mass instead of the raw bounding box.
