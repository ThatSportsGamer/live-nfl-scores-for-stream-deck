...
- **Gamecast shortcut** — press any key to open that game directly in ESPN Gamecast
...
**v1.0.11.0**
- Fixed every key showing `Err`: ESPN changed its scoreboard API to reject the multi-day `dates=<range>` query this plugin used to pull a team's ±10-day window in one request — even a single-day range now gets a flat HTTP 400. Rebuilt the same coverage (last week's final, this week, next week) from up to three requests scoped to `week` + `seasontype` instead, the one query shape ESPN still accepts, merged into one event list and cached for 20 seconds so multiple keys share the same fetch instead of multiplying requests.

**v1.0.10.0**
- Added a custom background color option in the settings panel: toggle it on, pick a color, and adjust its opacity. Useful for telling teams apart at a glance when you've got several keys configured (e.g. your Falcons key in red, your Panthers key in blue). Off by default — keys keep the original plain black background unless you turn it on. Applies to the normal score display; the score-flash and end-of-game fireworks animations still use their own colors on top of it.
...
- That same carry-forward is deliberately skipped during halftime — ESPN's possession data is blank there too, but for 15-20 real minutes with genuinely nobody holding the ball, so the key correctly shows no possession color through the whole intermission instead of freezing on whoever had it at the two-minute warning.
...
**v1.0.1.0**
- Fixed a bug where ESPN's edge network (Akamai) would return a `403 Access Denied` HTML page instead of JSON for requests that didn't look like a real browser, which showed up on the key as an `Err` state. Requests now send a realistic browser header set (User-Agent, Accept, Accept-Encoding) and transparently decompress the gzip/brotli response that comes back as a result.
...
BYE WEEK
```
Pressing the key in this state opens that team's schedule on ESPN.
